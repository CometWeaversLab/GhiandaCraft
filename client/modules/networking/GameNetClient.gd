...
func requestTerrainData(chunk_x: int, chunk_z: int):
	_requestTerrainData(chunk_x, chunk_z);
...
func _sendRequest(packetBytes: PackedByteArray):
	tcpConnection.poll(); # FIXME devo forzare il primo polling per motivi di test
	var tcpConnectionStatus = tcpConnection.get_status();
	if(tcpConnectionStatus != StreamPeerTCP.STATUS_CONNECTED):
